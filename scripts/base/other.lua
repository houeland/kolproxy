...
function betatest_parse_outfits()
	local outfits = {}
	...
	for l in io.lines("cache/files/modifiers.txt") do
		...
		if name and bonuslist and outfits[name] then
			...
			for x, y in (", "..bonuslist):gmatch(", ([^,:]+): ([^,]+)") do
				if checks[x] then
					outfits[name].bonuses[checks[x]] = tonumber(y)
				end
			end
			if next(outfits[name].bonuses) then
				pruned_outfits[name] = outfits[name]
			end
		end
	end
        local f = io.open("scripts/custom/twillo/outfits-candidate.lua", "w")
        f:write("outfits_modifier_data = " .. tostring(pruned_outfits).."\n")
        f:close()
end

function betatest_parse_buffs()
	local pruned_buffs = {}
	local section = nil
	local blacklist = {
		["Expert Timing"] = true,
	...
	for l in io.lines("cache/files/modifiers.txt") do
		...
		local name, bonuslist = l:match([[^([^	]+)	(.+)$]])
		if section == "Status Effects" and name and bonuslist then
			local bonuses = {}
			local checks = {
				["Initiative"] = "initiative",
				["Item Drop"] = "item",
			...
			if next(bonuses) and not blacklist[name] then
				pruned_buffs[name] = bonuses
			end
		end
	end
        local f = io.open("scripts/custom/twillo/buffs-candidate.lua", "w")
        f:write("buffs_modifier_data = " .. tostring(pruned_buffs).."\n")
        f:close()
end
