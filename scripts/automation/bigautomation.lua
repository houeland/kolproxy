...
if monstername gaudy pirate
  use Rain-Doh black box
	...
	local function ensure_yellow_ray()
		...
		if not have("unbearable light") and not cached_stuff.tried_to_summon_unbearable_light then
...
-- 	print("turns_to_next_sr", turns_to_next_sr)
	...
	have_reagent_pastas = (whichday - 1) * 2 + count ("knoll mushroom") count("hellion cube") + count("goat cheese") + count("Hell ramen") + count("Hell broth") + count("fettucini Inconnu") + count("fancy schmancy cheese sauce")
	if ascensionstatus() ~= "Hardcore" then
...
-- 	print("reagent pasta have", have_reagent_pastas, "need", need_total_reagent_pastas)
	...
	add_task {
		...
		action = function()
			...
			script.ensure_buffs { "Ode to Booze" }
			if not have("fortune cookie") or count("pumpkin beer") < 3 or buffturns("Ode to Booze") < 5 then
...
endwhile
	...
	add_task {
		prereq = challenge == "fist" and drunkenness() < estimate_max_safe_drunkenness() and have("astral pilsner") and level() >= 11,
		f = function()
...
--	}
	...
	add_task {
		when = not have("barrel of gunpowder") and level() >= 8 and level() < 12 and script.have_familiar("Obtuse Angel"),
		task = {
		...
		f = function()
			...
			if covept:match("The F'c'le") then
				...
				elseif have("Orcish Frat House blueprints") and quest_text("and asked you to steal his dentures back") then
					...
					if not have("frilly skirt") and moonsign_area() ~= "Degrassi Knoll" and not ascensionstatus("Hardcore") then
						if challenge == "boris" then
