...
decodeError _ = error "impossible decodeError value"
...
decodeColumnType _ = error "impossible decodeColumnType value"
...
foreign import ccall "sqlite3_busy_timeout"
  busy_timeoutC :: Ptr () -> Int -> IO Int
...
open path = do
  ...
  case databaseOrError of
    Left (Database database) -> do
      busy_timeoutC database 10000
...
bindTextError :: Statement -> Int -> BS.ByteString -> IO Error
bindTextError (Statement statement) parameterIndex byteString = do
  size <- return $ BS.length byteString
  BS.useAsCString byteString
                  (\dataC -> do
                     error <- bindTextC statement parameterIndex dataC size
                                        (intPtrToPtr (-1))
                     return $ decodeError error)
bindText :: Statement -> Int -> BS.ByteString -> IO ()
...
columnText :: Statement -> Int -> IO BS.ByteString
